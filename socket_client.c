...
void jouerPartie(int *sockfd, char *buffer) {
  ...
  while (!finDePartie) {
    ...
    } else if (strstr(buffer, "FIN2") != NULL || strstr(buffer, "IN2") != NULL) {

      printf("Le joueur défié a gagné ! Félicitations !\n");
      finDePartie = true;
      break;

    } else if (strstr(buffer, "FIN3") != NULL || strstr(buffer, "IN3") != NULL) {

      printf("Personne n'a gagné, égalité.\n");
      finDePartie = true;
      break;

    } else if (strstr(buffer, "FINDEC") != NULL || strstr(buffer, "INDEC") != NULL) {
